...
::applyAlignmentsToGeometry()
{
  ...
  if (doTracker_) {
    ...
    std::auto_ptr<AlignmentErrorsExtended>      alignmentErrors(theTrackerAlignables->alignmentErrors());
  ...
  if (doMuon_) {
    ...
    std::auto_ptr<AlignmentErrorsExtended> dtAlignmentErrorsExtended( theMuonAlignables->dtAlignmentErrorsExtended());
    std::auto_ptr<AlignmentErrorsExtended> cscAlignmentErrorsExtended(theMuonAlignables->cscAlignmentErrorsExtended());
...
::writeForRunRange(cond::Time_t time)
{
  ...
  if (doTracker_) {
    ...
    AlignmentErrorsExtended* alignmentErrors = theTrackerAlignables->alignmentErrors();
  ...
  if (doMuon_) {
    ...
    AlignmentErrorsExtended* alignmentErrors = theMuonAlignables->dtAlignmentErrorsExtended();

    writeDB(alignments, "DTAlignmentRcd",
            alignmentErrors, "DTAlignmentErrorExtendedRcd",
            muonGlobal,
            time);
    
    // Get alignments+errors, now CSC - ownership taken over by writeDB(..), so no delete
    alignments      = theMuonAlignables->cscAlignments();
    alignmentErrors = theMuonAlignables->cscAlignmentErrorsExtended();

    writeDB(alignments, "CSCAlignmentRcd",
            alignmentErrors, "CSCAlignmentErrorExtendedRcd",
            muonGlobal,
            time);
  }
}
